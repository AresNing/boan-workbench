...
Boan 以项目和成果组织 AI 编程工作。每项任务集中保存目标、约束、进展、决定、文件和验证证据，适用于修复问题、开发功能、更新文档，以及同时跟进多个本机项目，无需整理独立会话。

1. **交代工作。** 选择项目、描述目标，按需添加附件或选择模型参数。
2. **自动推进。** 工作台衔接执行与验证，可随时补充要求或暂停；业务决定和权限仍由你掌握。
3. **验收成果。** 查看文件与验证证据，确认完成或提出修改。

![英文工作台：CSV 导出成果、验证状态与任务输入框](docs/images/workbench-en.png)

*工作台将成果、验证和验收操作集中展示。所有截图均由实际界面配合虚构英文测试数据渲染，项目名、路径、模型标签与成果均为示例，不含真实账号或私人项目数据。*
...
**任务看板：** 按待处理、推进中、已完成查看任务，点击任务即可回到相应工作上下文。

![英文任务看板：按状态聚合的示例任务](docs/images/task-board-en.png)

**所有项目：** 汇总跨项目的待决定事项和成果，直接切换项目，其他项目继续后台推进。支持浅色与深色外观。

![浅色英文项目总览：三个示例项目](docs/images/projects-en.png)
